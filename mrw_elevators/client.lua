			...
			if Config.ThirdEyeName == 'ox_target' then
				...
				exports.ox_target:addBoxZone({
				    ...
				    options = {
					{
					    ...
					    label = "Použít výtah z " .. floor.level,
					    onSelect = function()
						TriggerEvent("mrw_elevator:showFloors",info)
					    end
					}
				    }
				})
        ...
        for elevatorName, elevatorFloors in pairs(Config.Elevators) do
			...
			sleep = 0
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            data.level = index
                            TriggerEvent('mrw_elevator:showFloors', data)
                        end
                    end
                end
            end
...
RegisterNetEvent("mrw_elevator:showFloors", function(data)
	local elevator = {}
	local floor = {}
	if Config.UseESX then
		PlayerData = ESX.GetPlayerData()
	...
	elseif Config.OXLib then
		lib.registerMenu({
			id = 'elevator_ox',
			title = 'Výtah pro zaměstnance',
			options = elevator,
			position = 'top-right',
		}, function(selected, scrollIndex, args)
			if not args.value2 then
				TriggerEvent("mrw_elevator:movement", args.value)
			else
				NotifyNoAccess()
			end
		end)
...
RegisterNetEvent("mrw_elevator:movement", function(arg)
	local floor = {}
